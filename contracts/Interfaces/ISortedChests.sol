...
// Common interface for the SortedChests Doubly Linked List.
interface ISortedChests {
    struct Dependencies {
    ...
    event NodeAdded(address _asset, address _id, uint256 _NICR);
    event NodeRemoved(address _asset, address _id);
    ...
    function validInsertPosition(uint256 _ICR, address _asset, address _prevId, address _nextId) external view returns (bool);

    function findInsertPosition(uint256 _ICR, address _asset, address _prevId, address _nextId) external view returns (address, address);
}
